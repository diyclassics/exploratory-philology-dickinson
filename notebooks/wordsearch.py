# B. Helmig's Crossword Puzzle Generator
...
class Crossword(object):
    def __init__(self, cols, rows, empty = '-', maxloops = 2000, available_words=[]):
        ...
        self.debug = 0
        self.clear_grid()
        self.latin_lower = string.ascii_lowercase.replace('j','').replace('v', '').replace('w', '')
        ...
        for r in range(self.rows):
            for c in self.grid[r]:
                if c == self.empty:
                    ...
                    outStr += '%s ' % random.choices(self.latin_lower, [9,1,4,3,11,1,1,1,11,1,3,5,6,5,3,1,8,7,8,8,1,1,1], k=1)[0]
                else:
                    outStr += '%s ' % c
            outStr += '\n'
        return outStr
